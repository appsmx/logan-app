...
// LOGAN OS — Panel del cliente (DEC-LOGAN-022, Fase 3.1 + 3.2).
//
// Página acotada para el DUEÑO del negocio: /panel/[projectId].
// - Si no está autenticado → muestra login (contraseña simple por proyecto).
// - Si está autenticado → muestra SOLO sus conversaciones + handoff bot/humano.
//
// No expone las secciones internas de LOGAN. Usa los endpoints /api/handoff/client/*
// que aíslan por projectId.
...
export default function ClientPanelPage({
  ...
  // Detecta si ya hay sesión intentando listar (401 → no autenticado).
  React.useEffect(() => {
...
function PanelInner({
  ...
  // Poll de conversaciones cada 5s.
  React.useEffect(() => {
    let alive = true;
    const load = () =>
