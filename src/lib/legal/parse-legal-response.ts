// LOGAN Legal — defensive parser. Same pattern as marketing/dev/design/finance (Art. III).

import type { LegalResponse, LegalHypothesis } from "@/lib/legal/types";
...
function asHypothesis(v: unknown): LegalHypothesis {
  const empty: LegalHypothesis = { context: "", hypothesis: "", prediction: "" };
  if (!v || typeof v !== "object") return empty;
  const o = v as Record<string, unknown>;
  return { context: asString(o.context), hypothesis: asString(o.hypothesis), prediction: asString(o.prediction) };
}

const FALLBACK_HYP: LegalHypothesis = {
  context: "El modelo no devolvió una hipótesis estructurada; placeholder para preservar DEC-LOGAN-004.",
  hypothesis: "Creemos que el documento legal propuesto mejorará la postura de cumplimiento del proyecto.",
  prediction: "La métrica legal clave (incidentes, validaciones, cumplimiento) será medible cuando se implemente la recomendación.",
};

export function parseLegalResponse(rawText: string): LegalResponse {
  const fallback: LegalResponse = {
    title: "Entregable legal",
    content: rawText.trim() || "(LOGAN Legal no devolvió contenido.)",
  ...
  const title = asString(obj.title, "Entregable legal");
  ...
  if (!hypothesis.context) hypothesis.context = `Legal: ${title}`;
