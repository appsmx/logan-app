...
import {
  ROLES, DESIGN_CAPABILITIES, DESIGN_ASSET_TYPES, HYPOTHESIS_STATUSES,
} from "@/lib/logan-os-data";
import { useCreateDesign, useDeleteDesign, useDesign } from "@/lib/hooks";
import { useLoganStore } from "@/lib/store";
import { toast } from "sonner";
import { shortDate } from "@/lib/api";
import { Palette, Plus, Trash2, Lightbulb, Link as LinkIcon } from "lucide-react";

const DESIGN_ROLE = ROLES.find((r) => r.key === "design")!;
...
export function DesignSection() {
  ...
  const list = useDesign(activeId);
  const del = useDeleteDesign(activeId ?? "");
  const [createOpen, setCreateOpen] = React.useState<string | null>(null);

  const items = list.data ?? [];

  return (
    <section className="space-y-6" aria-labelledby="design-title">
      <SectionHeading
        eyebrow="Rol especialista · Activo"
        title="Design"
        icon="Palette"
        description="El especialista en diseño de producto y experiencia de usuario. Dueño del sistema visual. Cada entregable lleva una hipótesis de diseño verificable."
      />

      <Card className="border-t-2 border-t-destructive/30">
        <CardHeader>
          <CardTitle className="flex items-center gap-3 font-serif text-lg">
            <span className="flex size-9 items-center justify-center rounded-lg bg-destructive/10 text-destructive ring-1 ring-destructive/25">
              <RoleIcon name={DESIGN_ROLE.icon} className="size-5" />
            </span>
            {DESIGN_ROLE.name}
          </CardTitle>
          <CardDescription>{DESIGN_ROLE.tagline}</CardDescription>
        </CardHeader>
        <CardContent>
          <ul className="grid gap-2 sm:grid-cols-3">
            {DESIGN_ROLE.responsibilities.map((r) => (
              <li key={r} className="flex items-start gap-2 rounded-md border bg-card/40 px-3 py-2 text-sm">
                <Palette className="mt-0.5 size-3.5 shrink-0 text-destructive/60" />
      ...
      {activeId ? (
        <>
              ...
              {DESIGN_CAPABILITIES.map((c) => (
                ...
                >
                  ...
                  <span className="mt-2 inline-flex w-fit items-center rounded-md bg-muted px-2 py-0.5 text-[10px] uppercase tracking-widest text-muted-foreground">
                    {DESIGN_ASSET_TYPES[c.producesAssetType]?.label ?? c.producesAssetType}
                  </span>
                </button>
              ))}
            </div>
          ...
          <div>
            <h3 className="mb-3 font-serif text-lg text-foreground">Entregables de Design</h3>
            {items.length === 0 ? (
              <EmptyState
                icon={<Palette className="size-5" />}
                title="Sin entregables todavía"
                description="Selecciona una capacidad para crear el primer entregable de diseño. Cada uno irá atado a una hipótesis."
              />
            ) : (
                      ...
                      <div className="flex w-full flex-col gap-2 pr-2 sm:flex-row sm:items-center sm:justify-between">
                        <div className="min-w-0 text-left">
                          <div className="flex flex-wrap items-center gap-1.5 text-[10px] uppercase tracking-widest text-muted-foreground">
                            <span>{DESIGN_ASSET_TYPES[a.type]?.label ?? a.type}</span>
                            <span>·</span>
        ...
        <EmptyState
          ...
          description="Design vive dentro de un proyecto. Crea uno para empezar a diseñar interfaces, sistemas visuales y flujos de usuario."
        />
      )}

      <CreateDesignDialog capabilityKey={createOpen} onOpenChange={(v) => !v && setCreateOpen(null)} />
    </section>
  );
}
...
function CreateDesignDialog({
  ...
  const activeId = useLoganStore((s) => s.activeProjectId);
  const create = useCreateDesign(activeId ?? "");
  const cap = DESIGN_CAPABILITIES.find((c) => c.key === capabilityKey);
  ...
  const submit = () => {
    ...
    create.mutate(
      { type: cap.producesAssetType, title: title.trim(), content: content.trim(),
        hypothesis: { roleId: "design", context: context.trim() || `Design: ${cap.label}`, hypothesis: hyp.trim(), prediction: prediction.trim() } },
      {
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto logan-scroll">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 font-serif">
            <Palette className="size-4 text-destructive" />{cap.label}
          </DialogTitle>
        ...
        <div className="space-y-4">
          <div className="grid gap-2">
            ...
            <Input id="ds-title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ej. Sistema de colores oklch para Mr. Trámite" />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="ds-content">Contenido del entregable</Label>
            <Textarea id="ds-content" value={content} onChange={(e) => setContent(e.target.value)} rows={10}
              placeholder="Especificación de UI, sistema visual, prototipo, handoff, auditoría… El entregable completo." />
          </div>
          <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 space-y-3">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-primary">
              <Lightbulb className="size-3.5" />Hipótesis de diseño
            </div>
            <p className="text-xs text-muted-foreground -mt-1">Todo entregable de Design va atado a una hipótesis de diseño verificable.</p>
            <div className="grid gap-2">
              <Label htmlFor="ds-ctx" className="text-xs">Contexto</Label>
              <Textarea id="ds-ctx" value={context} onChange={(e) => setContext(e.target.value)} rows={2}
                placeholder="¿Qué problema de UX o decisión visual disparó este entregable?" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="ds-hyp" className="text-xs">Hipótesis *</Label>
              <Textarea id="ds-hyp" value={hyp} onChange={(e) => setHyp(e.target.value)} rows={2}
                placeholder="Creemos que este diseño mejorará X porque Y." />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="ds-pred" className="text-xs">Predicción medible *</Label>
              <Textarea id="ds-pred" value={prediction} onChange={(e) => setPrediction(e.target.value)} rows={2}
                placeholder="Tasa de completitud > X%, tiempo de tarea < Xs, error rate < X%…" />
            </div>
          </div>
        </div>
        <DialogFooter>
